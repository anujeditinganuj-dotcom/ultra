...
def _register_owner_gate(clone: Client, owner_id: int):
    async def _gate(client, update):
        ...
        # Not the owner: actually stop it here. Pyrogram runs every group by
        # default regardless of what a handler returns — only stop_propagation()
        # (or raising StopPropagation) prevents the real handlers further down
        # from also seeing this update. Without this call, this branch did
        # nothing and every non-owner's messages/button-taps still reached the
        # clone's real handlers same as the owner's.
...
def register_titanium_handlers(app, *, make_button, BTN_PRIMARY, BTN_DANGER, SC, ParseMode,
    ...
    async def _titanium_cmd(client, m: Message):
        logger.info(f"DEBUG: _titanium_cmd ENTERED — user={m.from_user.id}, chat={m.chat.id}")
        try:
    ...
    async def _titanium_ping_cb(client, query: CallbackQuery):
        logger.info(f"DEBUG: _titanium_ping_cb ENTERED — user={query.from_user.id}")
    ...
    async def _addbot_cmd(client, m: Message):
        logger.info(f"DEBUG: _addbot_cmd ENTERED — user={m.from_user.id}, chat={m.chat.id}, text={m.text!r}")
