...
from diskwala import fetch_diskwala_video, extract_diskwala_links, DiskwalaAuthError, fetch_playlist_info, is_playlist_link, fetch_diskwala_preview
...
async def process_link(client: Client, m: Message, link: str, tag: str):
    ...
    # Title + thumb → API (reliable)
    # Author + duration → HTML scrape (API doesn't provide these)
    ...
    dur_secs = preview.get("duration_seconds") or 0
    views       = preview.get("views")
    likes       = preview.get("likes")
    description = preview.get("description")
    upload_date = preview.get("upload_date")
