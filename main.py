...
from diskwala import fetch_diskwala_video, extract_diskwala_links, DiskwalaAuthError, fetch_playlist_info, is_playlist_link
...
async def _download_video_inner(client: Client, query, link: str, quality: str, status_msg, chat_id: int):
    if True:
        ...
        try:
            ...
            # Preview card (poster + title + quality) as soon as the link
            # resolves, before the download itself starts — a separate
            # message since status_msg is text-only and can't be turned
            # into a photo message via edit.
            preview_title = video_info.get("name") or name
            preview_caption = (
                f"🚀 <b>{preview_title}</b>\n\n"
                "🎞 <b>Quality:</b> Best Available"
            )
            if thumb_url:
                try:
                    await client.send_photo(chat_id, thumb_url, caption=SC(preview_caption), parse_mode=ParseMode.HTML)
                except Exception as e:
                    logger.warning(f"Preview photo failed, falling back to text: {e}")
                    await client.send_message(chat_id, SC(preview_caption), parse_mode=ParseMode.HTML)
            else:
                await client.send_message(chat_id, SC(preview_caption), parse_mode=ParseMode.HTML)
