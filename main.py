...
# TEMPORARY DEBUG — proves whether an update reaches the dispatcher at all,
# independent of any filter matching further down. Remove once the
# addbot/delbot/titanium/ping mystery is solved.
@app.on_message(filters.all, group=-50)
async def _debug_catch_all_messages(client: Client, m: Message):
    try:
        logger.info(
            f"DEBUG catch-all MESSAGE: chat={m.chat.id} ({m.chat.type}), "
            f"user={m.from_user.id if m.from_user else None}, text={m.text!r}"
        )
    except Exception as e:
        logger.info(f"DEBUG catch-all MESSAGE logging failed: {e}")


@app.on_callback_query(filters.all, group=-50)
async def _debug_catch_all_callbacks(client: Client, query):
    try:
        logger.info(f"DEBUG catch-all CALLBACK: user={query.from_user.id}, data={query.data!r}")
    except Exception as e:
        logger.info(f"DEBUG catch-all CALLBACK logging failed: {e}")
