...
# deploy reuses the existing auth key instead of calling
# auth.ImportBotAuthorization again — that repeated call is what Telegram
# rate-limits (FloodWait) when redeploys happen close together. Leave
# unset on first run; main.py logs a string to copy in here once the bot
# starts successfully.
BOT_SESSION_STRING = os.getenv("BOT_SESSION_STRING", "").strip() or None
